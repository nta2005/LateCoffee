...
public class MyCartAdapter extends RecyclerView.Adapter<MyCartAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.txt_food_price.setText(new StringBuilder().append(Common.formatPrice(cartItemList.get(position).getFoodPrice() + cartItemList.get(position).getFoodExtraPrice())).append(" đ"));

        if (cartItemList.get(position).getFoodSize() != null) {
            if (cartItemList.get(position).getFoodSize().equals("Mặc định"))
                holder.txt_food_size.setText(new StringBuilder("Size: ").append("Mặc định"));
            else {
        ...
        if (cartItemList.get(position).getFoodAddon() != null) {
            if (cartItemList.get(position).getFoodAddon().equals("Mặc định"))
                holder.txt_food_addon.setText(new StringBuilder("Thêm: ").append("Mặc định"));

            else {
                List<AddonModel> addonModels = gson.fromJson(cartItemList.get(position).getFoodAddon(),
                        new TypeToken<List<AddonModel>>() {
                        }.getType());
                holder.txt_food_addon.setText(new StringBuilder("Thêm: ").append(Common.getListAddon(addonModels)));
            }
        }

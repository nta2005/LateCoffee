...
public class FoodListFragment extends Fragment {
    ...
    private MySwipeHelper swipeButton;
    ...
    private void startSearchFood(String s) {
        ...
        for (int i = 0; i < Common.categorySelected.getFoods().size(); i++) {
            ...
            if (foodModel.getName().toLowerCase().contains(s.toLowerCase())) {
                foodModel.setPositionInList(i); //Save index for item after search found data
    ...
    private void initViews() {
        ...
        //Change title of toolbar when we select category
        ...
        swipeButton = new MySwipeHelper(getContext(), recycler_food_list, width / 6) {
            @Override
            public void instantiateMyButton(RecyclerView.ViewHolder viewHolder, List<MyButton> buf) {
                ...
                //Button Size
                buf.add(new MyButton(getContext(), "Size", 35, 0, Color.parseColor("#12005e"),
                        pos -> {
                            FoodModel foodModel = adapter.getItemAtPosition(pos);
                            if (foodModel.getPositionInList() == -1)
                                Common.selectedFood = foodModelList.get(pos);
                ...
                //Button Addon
                buf.add(new MyButton(getContext(), "Thêm", 35, 0, Color.parseColor("#336699"),
                        pos -> {
                            FoodModel foodModel = adapter.getItemAtPosition(pos);
                            if (foodModel.getPositionInList() == -1)
                                Common.selectedFood = foodModelList.get(pos);
    ...
    private void errorDialog() {
        ...
        builder1.setMessage("Thông tin không được để trống");
    ...
    private void showUpdateDialog(int pos, FoodModel foodModel) {
        ...
        EditText edt_food_des = itemView.findViewById(R.id.edt_food_description);
        ...
        builder.setPositiveButton("Cập nhật", (dialogInterface, i) -> {
            ...
            if (edt_food_name.getText().toString().trim().isEmpty()) {
                errorDialog();
            } else if (edt_food_des.getText().toString().isEmpty()) {
                errorDialog();
    ...
    private void showAddDialog() {
        ...
        //Set data
        Glide.with(getContext()).load(R.drawable.ic_default).into(img_food);
        ...
        builder.setNegativeButton("Đóng", ((dialogInterface, i) -> dialogInterface.dismiss()))
                .setPositiveButton("Thêm", ((dialogInterface, i) -> {
                    ...
                    updateFood.setName(edt_food_name.getText().toString().trim());
                    updateFood.setDescription(edt_food_description.getText().toString());
                    updateFood.setPrice(TextUtils.isEmpty(edt_food_price.getText()) ? 0 :
                            Long.parseLong(edt_food_price.getText().toString()));


                    //hiển add đoạn nầy hiển đẹp zai bodoi thế :)))
                    if (edt_food_name.getText().toString().trim().isEmpty()) {
                        errorDialog();
                    } else if (edt_food_description.getText().toString().isEmpty()) {
                        errorDialog();
                    } else if (imageUri == null) {
